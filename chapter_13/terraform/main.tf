...
resource "aws_iam_user" "chap_13_on_prem_user" {
    name = "chap-13-on-prem-user"
}
resource "aws_iam_user_policy_attachment" "attach_code_deploy" {
  user       = aws_iam_user.chap_13_on_prem_user.name
...
resource "aws_iam_role" "chap_13_pipeline_role" {
  name                = "chap-13-pipeline-role"
...
resource "aws_iam_instance_profile" "chap_13_instance_profile" {
  name = "chap_13_deploy_instance_profile"
  role = aws_iam_role.chap_13_pipeline_role.name
}

// EC2 Auto scalling Group 
resource "aws_autoscaling_group" "chap_13_asg" {
  name               = "chap-13_asg"
  min_size           = 1
  max_size           = 1
  desired_capacity   = 1
  ...
  launch_template {
    id      = aws_launch_template.chap_13_ec2_launch_template.id
    version = "$Latest"
  }
...
resource "aws_launch_template" "chap_13_ec2_launch_template" {
  instance_type = "t2.micro"
  image_id      = "ami-0ed9277fb7eb570c9"
  name          = "chap_13-ec2-launch-template"
  key_name      = "packt_key"
  iam_instance_profile {
    name = aws_iam_instance_profile.chap_13_instance_profile.name
  }
  security_group_names = [aws_security_group.chap_13_ins_sg.name]
  user_data = filebase64("user_data.sh")
}


// Security group for EC2 instance to provide access to http,https and ssh port 
resource "aws_security_group" "chap_13_ins_sg" {
  description = "security group for http/https and ssh access"
  name        = "chap_13-instance-security-group"
  ingress {
...
resource "aws_iam_role" "chap_13_code_deploy_role" {
  name                = "chap-13-code-deploy-role"
  assume_role_policy  = <<EOF
{
    "Version": "2012-10-17",
    "Statement": [
...
*/
resource "aws_codedeploy_app" "chap-13-on-prem-code-deploy" {
  name             = "chap-13-on-prem-code-deploy"
...
*/
resource "aws_codedeploy_deployment_group" "chap-13-on-prem-deploy-group" {
  app_name               = aws_codedeploy_app.chap-13-on-prem-code-deploy.name
  deployment_group_name  = "chap-13-on-prem-deploy-group"
  deployment_config_name = "CodeDeployDefault.AllAtOnce"
  service_role_arn       = aws_iam_role.chap_13_code_deploy_role.arn
